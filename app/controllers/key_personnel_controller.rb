# -*- coding: utf-8 -*-
class KeyPersonnelController < SecuredController
  layout false
  ...
  def lookup
    ...
    begin
      ...
      add_user(applicant) if applicant.new_record?
      key = add_key_person(applicant, params[:submission_id], params[:role])
      render :update do |page|
        page.replace_html(dom_id,
                          partial: 'shared/personnel_fields',
                          layout: false,
                          locals: { key_person: key, submission: submission, index: params[:index] })
      end
