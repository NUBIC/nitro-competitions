module ProjectsHelper
  ...
  def project_status
    ...
    current_projects.each do |project|
      ...
      unless 1 == 1 or project.project_period_start_date.nil?
  ...
  def project_will_start(start_date, phrase)
    if Time.now < start_date and Time.now + 90.days > start_date
      proj_heading(phrase+ " process  will open in " + distance_of_time_in_words(Time.now,start_date) + ".")
    end
  end
  def project_is_ongoing(start_date, end_date, phrase)
    if Time.now + 1.day > start_date and Time.now - 1.day < end_date
      proj_heading(phrase+ " process is ongoing. The process started "+distance_of_time_in_words(Time.now,start_date) + " ago and will close in "+ distance_of_time_in_words(Time.now,end_date) + ".")
    end
  end
  def project_recently_closed(end_date, phrase)
    if Time.now > end_date and Time.now < end_date + 21.days
      proj_heading(phrase+ " process was closed "+ distance_of_time_in_words(Time.now,end_date) + " ago.")
    end
  end
