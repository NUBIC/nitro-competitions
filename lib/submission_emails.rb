...
def send_finalize_email(submission, user)
  if allow_emails? or user.username == 'wakibbe'
    ...
    begin
      Notifier.deliver_finalize_message("wakibbe@northwestern.edu", "Thank you for your NUCATS Assist Submission", submission, submission_url(submission.id), project_url(submission.project.id))
      submission.save!
      begin
